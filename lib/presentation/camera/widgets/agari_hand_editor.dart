...
// 戻り値の型を Future<List<String>?> に変更し、asyncキーワードを追加
Future<List<String>?> showAgariHandDialog(
  BuildContext context,
  WidgetRef ref,
) async {
  final handState = ref.watch(handProvider);
  print('Current hand state: $handState');

  // showModalBottomSheetの戻り値（編集後の手牌リスト）を受け取る
  ...
  // 受け取った値を返す（ダイアログがキャンセルされた場合はnullが返る）
  return result;
}

class AgariHandEditor extends StatefulWidget {
  final List<String> initialHand;
  const AgariHandEditor({super.key, required this.initialHand}); // keyの追加を推奨
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              GestureDetector(
                // 完了ボタンが押されたら、編集後の手牌リストをpopの引数で渡す
